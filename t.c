...
// Assuming landers are stored in sprites[2] and sprites[3]
void Draw_all() {
    char *p;
    int i, j;
    ...
    if (projectilesActive) {
        ...
        for (i = 0; i < 10; i++) {
            if (projectileList[i].active) {
                show_bmp(projectileList[i].p, projectileList[i].y, projectileList[i].x);
            }
        }
    }

    // Draw the defender (assuming it's at sprites[1])
    show_bmp(sprites[1].p, sprites[1].y, sprites[1].x);
    ...
    // Reset the flag indicating projectiles are active
    projectilesActive = 0;
...
//     uprintf("got vidhandler %x counter %d\n",*(volatile unsigned int *)(0x1012001C),counter);
     ...
     switch(key){
         ...
         break;
       case 's':
                // Logic for moving the defender down
           sprites[1].y += 16;  
           spriteMove = 1;  // Signal that a sprite has moved
           break;
       case 'e':
                // Logic for moving the defender up
           sprites[1].y -= 16;  
           spriteMove = 1;  // Signal that a sprite has moved
           break;
       case 'w':
            // Logic for moving the defender left
           sprites[1].x -= 16;  
           spriteMove = 1;  // Signal that a sprite has moved
           break;
       case 'f':
           ...
           if (!firing) {
   	     ...
   	     // For example, create sprite a new for the fired projectile

              sprites[4].x = sprites[1].x + 10;  
              sprites[4].y = sprites[1].y;     
              sprites[4].p = &_binary_shoot_bmp_start;  // Assuming shoot_bmp is the projectile sprite

        	// Set flags and update variables accordingly
              firing = 1;          // Setting firing flag
              spriteMove = 1;      // Signal that a sprite has moved
              projectilesActive = 1;  // Setting a flag indicating that projectiles are active
      ...
      // Checking for collisions with landers
if (firing) {

    for (int i = 2; i < 4; i++) {  // Assuming landers are stored in sprites[2] and sprites[3]
        if (sprites[i].enabled &&
            sprites[4].x < sprites[i].x + landerWidth &&
            sprites[4].x + projectileWidth > sprites[i].x &&
            sprites[4].y < sprites[i].y + landerHeight &&
            sprites[4].y + projectileHeight > sprites[i].y) {
            // Collision detected with lander i, handling it by removing the lander.
